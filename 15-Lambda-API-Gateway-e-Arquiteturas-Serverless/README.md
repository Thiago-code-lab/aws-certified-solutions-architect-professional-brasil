...
Este módulo cobre APIs, eventos, escala sob demanda, limites operacionais e custo por uso, com foco no tipo de raciocínio arquitetural exigido no AWS Certified Solutions Architect Professional (SAP-C02).

## Conceitos-Chave

- Domínio predominante: Design for New Solutions.
- Serviços e padrões principais: Lambda, API Gateway, EventBridge, Step Functions, DynamoDB.
...
Para revisar Lambda, API Gateway e DynamoDB em arquiteturas serverless antes de aprofundar APIs serverless, limites e operação por eventos:
